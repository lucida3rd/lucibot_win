...
#### 監視情報取得機能【 \g 】
Twitterでおこなっているいいね、フォロー、フォロワーの状態を取得、その他自動実行をします。
取得したあとデータベースに保存され、あとで閲覧することができます。

実行される処理：
・一定期間を過ぎたいいねを自動でリムーブします。
・normalリストに登録されているユーザをフォローしてから一定期間を過ぎてリフォローされなかった場合、自動でリムーブします。
・normalリストに登録されているユーザのうち、いいねしたのに一定期間いいね、リプライをもらえなかった場合、自動でリムーブします。
・荒らしユーザを監視し、CSVに出力します。
